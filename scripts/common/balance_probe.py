...
from analyze_gate import FusionProbe, load_model, to_device
...
for entry in sys.argv[1:]:
    ...
    try:
        ...
        gen = torch.Generator(device=device); gen.manual_seed(20260820)
        with torch.inference_mode():
            for i, batch in enumerate(loader):
                if i >= 30: break
                batch = to_device(batch, device)
    ...
    print(f"{name:8}  camera share of attention mass {a:.3f}   camera share of sensitivity {b:.3f}")
