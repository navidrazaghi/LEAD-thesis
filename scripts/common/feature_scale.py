...
for entry in sys.argv[1:]:
        ...
        with torch.inference_mode():
            for i, batch in enumerate(loader):
                ...
                for x in seen:
                    ...
                    lid.append(f[:, IMAGE_TOKENS:IMAGE_TOKENS + BEV_TOKENS]
    ...
    if not cam:
        ...
        continue
    c, l = sum(cam)/len(cam), sum(lid)/len(lid)
    print(f"{name:8}  mean token norm  image {c:8.3f}   BEV {l:8.3f}   ratio BEV/image {l/c:6.2f}")
