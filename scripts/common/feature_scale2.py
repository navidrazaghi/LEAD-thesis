...
for entry in sys.argv[1:]:
    ...
    for m in model.modules():
        ...
        handles.append(m.ln1.register_forward_hook(
            lambda _m, _i, out, s=after: s.append(out.detach())))
        handles.append(m.attn.value_proj.register_forward_hook(
    ...
    def ratio(sink):
        r = []
        for x in sink:
            if x.shape[1] < IMG + BEV: continue
            f = x.float()
            c = f[:, :IMG].norm(dim=-1).mean().item()
            l = f[:, IMG:IMG + BEV].norm(dim=-1).mean().item()
            if c > 0: r.append((c, l))
        return r
    try:
        with torch.inference_mode():
            for i, batch in enumerate(loader):
                if i >= 15: break
                before.clear(); after.clear(); values.clear()
                ...
                with autocast(device_type="cuda", dtype=cfg.training.optimization.torch_dtype,
                    ...
                    model(batch)
                if i == 0:
                    stages = {"block input": ratio(before), "after ln1": ratio(after),
                              "after value_proj": ratio(values)}
                    print(f"\n{name}")
                    for label, r in stages.items():
                        if not r: print(f"  {label:18} (not captured)"); continue
                        c = sum(a for a, _ in r) / len(r)
                        l = sum(b for _, b in r) / len(r)
                        print(f"  {label:18} image {c:8.3f}  BEV {l:8.3f}  ratio {l/c:5.2f}")
                    break
    finally:
