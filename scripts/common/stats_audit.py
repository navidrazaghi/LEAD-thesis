...
by = {}
for r in rows:
    by[(r["model"], r["modality"] + ":" + r["severity"], r["route"])] = float(r["driving_score"])
...
for cond in conds:
    ...
    for m in ("rung0", "rung4"):
        ...
        n = len(d); mean = statistics.mean(d); sd = statistics.stdev(d)
        ...
        results.append({"comp": f"{m} vs rung2a, {cond}", "n": n, "mean": mean,
                        "t": t, "p": p, "wp": wilcoxon_p(d),
                        "blo": lo, "bhi": hi})

print("  comparison                          n    mean      p(t)   p(wilcoxon)  bootstrap 95%")
for r in results:
    print(f"  {r['comp']:34} {r['n']:2} {r['mean']:+7.2f}  {r['p']:.4f}   {r['wp']:.4f}      "
          f"[{r['blo']:+6.2f},{r['bhi']:+6.2f}]")

print()
print("  HOLM step-down over the family of six")
order = sorted(results, key=lambda r: r["p"])
k = len(order); prev = 0.0
for i, r in enumerate(order):
    adj = min(1.0, max(prev, (k - i) * r["p"]))
    prev = adj
    print(f"    {r['comp']:34} raw p={r['p']:.4f}  Holm p={adj:.4f}  "
