...
#
# The log path is a field of the governor's own, not evaluation.save_path. That
# one is a derived property fed by an environment variable, so passing it in a
# config dotlist raises inside the child before the agent exists -- which
# surfaces as "Agent couldn't be set up" on every route, with no traceback in
# the sweep's own log. It cost a twenty-route run that failed identically
# twenty times.
#
...
~/miniconda3/envs/lead/bin/python scripts/common/run_evaluation.py \
  ...
  --config evaluation.inference.use_caution_governor=true \
           ...
           evaluation.inference.caution_calibration_log="$OUT/caution_calibration.jsonl" \
