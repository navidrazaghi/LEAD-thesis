...
## And then it drove worse

This is the finding that was not being looked for.

Scored on the same protocol, the retrained baseline times out on **36%** of
routes against **3%** for the diverged run it replaces and **0%** for `rung2a`.
Routes the old model finished in two minutes it drives for forty-five without
finishing.
